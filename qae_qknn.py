...
class QuantumAutoencoder:
    ...
    def _build_qnodes(self):
        ...
        @qml.qnode(self.dev, interface="autograd")
        def latent_statevector_qnode(x, params):
            qml.AngleEmbedding(x, wires=range(cfg.n_qubits), rotation="Y")
            self._encoder_block(params)
            return qml.state()
        # Returns latent statevector too
        return trash_probs_qnode, latent_qnode, swap_test_prob_qnode, latent_statevector_qnode
    
    # Extracts the latent state to use for qknn
    @staticmethod
    def extract_latent_state(full_state, n_latent):
      n_total = int(np.log2(len(full_state)))
      state_tensor = full_state.reshape([2] * n_total)
      
      latent_dim = 2 ** n_latent
      trash_dim = 2 ** (n_total - n_latent)
      
      state_tensor = state_tensor.reshape(latent_dim, trash_dim)
      latent_state = np.linalg.norm(state_tensor, axis=1)
      return latent_state / (np.linalg.norm(latent_state) + 1e-12)
        
    def get_all_latent_states(self, X_raw, params, scaler):
      X = self.transform_to_angles(X_raw, scaler)
      latent_dim = 2**self.config.n_latent
      states = []
      
      for x in X:
          full_state = self.latent_statevector_qnode(x, params)
          latent_part = self.extract_latent_state(full_state, self.config.n_latent)
          states.append(latent_part)
